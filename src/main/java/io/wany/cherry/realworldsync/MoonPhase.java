...
public class MoonPhase {
  ...
  private final String[] moon_phase_name = {"New moon",          // 0
    "Waxing crescent",  // 1
    "First quarter",  // 2
    "Waxing gibbous",  // 3
    "Full moon",    // 4
  ...
  */
  ...
  public static int days_of_february(int year) {
    int day;
    if ((year > greg[YEAR]) || ((year == greg[YEAR]) && (greg[MONTH] == 1 || ((greg[MONTH] == 2) && (greg[LAST_DAY] >= 28))))) {
  ...
  public static double calendarToJD(Calendar cal) {
    ...
    return (((long) (365.25 * (y + 4716))) + ((int) (30.6001 * (m + 1))) + mday + b - 1524.5) + ((sec + 60L * (min + 60L * hour)) / 86400.0);
  }
  ...
  private static int computePhaseIndex(Calendar cal) {
    ...
    int[] day_year = {-1, -1, 30, 58, 89, 119, 150, 180, 211, 241, 272, 303, 333};
    ...
    double day_exact = day + hour / 24.0 + min / 1440.0 + sec / 86400.0;
  ...
  public String getMoonAgeAsDays() {
    ...
    return "" + aom_d + (aom_d == 1 ? " day, " : " days, ") + aom_h + (aom_h == 1 ? " hour, " : " hours, ") + aom_m + (aom_m == 1 ? " minute" : " minutes");
  }
  ...
  private static Calendar adjustTimeZone(Calendar c, int offsetInHours) {
    ...
    c.setTime(new Date(currTime + (long) offsetInHours * 1000 * 60 * 60));
    return c;
  }
