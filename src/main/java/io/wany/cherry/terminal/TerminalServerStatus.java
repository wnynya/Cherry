...
public class TerminalServerStatus {
  ...
  private static final Timer TPSTimer = new Timer();
  ...
  public static void sendSystem() {
    ...
    data.put("tps", lastTPS);
    Terminal.send(event, message, data);
  }

  public static void onEnable() {
    TPSTask = Bukkit.getScheduler().runTaskTimerAsynchronously(Cherry.PLUGIN, new Runnable() {
      @Override
      public void run() {
        TPS++;
      }
    }, 0L, 1L);
    sendServerStatusExecutorService.submit(new Runnable() {
      @Override
      public void run() {
